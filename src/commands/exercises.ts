...
Examples:
  ...
  strong exercises --json                   # machine-readable`,
    ...
    .action(
      async (options: { search?: string; user?: boolean; limit: string; fresh?: boolean }) => {
        ...
        logVerbose(
          ctx,
          options.fresh ? 'Refreshing exercise definitions...' : 'Fetching exercise definitions...',
        )
        const global = await client.getAllMeasurements({ fresh: options.fresh })
